...
object Anagrams {
  ...
  def wordAnagrams(word: Word): List[Word] = dictionaryByOccurrences.getOrElse(wordOccurrences(word),throw new Error("No such word"))
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    if(occurrences.isEmpty) List(List())
      ...
      val a = for {
        ...
        com <- b
      } yield List((occurrences.head._1,j)):::com
      a.toList:::b
    }
  }
...
//      val group = combinations(occurrences)
//      for {
//        a <- group
//        if(a.nonEmpty)
//        if(dictionaryByOccurrences.exists(_._1==a))
//        ls = occurAnagrams(subtract(occur,a))
//        if(ls.nonEmpty)
//        s <- ls
//        w <- dictionaryByOccurrences.getOrElse(a , throw new Error(""))
//      } yield w::s
//    }
//
//    occurAnagrams(occur)
  ...
  def sentenceAnagramsHelper(senOcc: Occurrences, senAcc: Sentence): List[Sentence] = senOcc match {
    ...
    case occur => {
      for {
        ...
        if com.nonEmpty
        if dictionaryByOccurrences.exists(_._1==com)
