...
package object barneshut {
  ...
  extends Quad {
    ...
    val (massX,massY) = (bodies.aggregate(0f)((z,b)=>z+b.mass*b.x,_+_)/mass : Float
      , bodies.aggregate(0f)((z,b)=>z+b.mass*b.y,_+_)/mass : Float)

    val total: Int = bodies.length

    def insert(b: Body): Quad = if(size>minimumSize) {
      val f = new Fork(Empty(centerX-size/4,centerY-size/4,size/2),Empty(centerX+size/4,centerY-size/4,size/2)
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      val deltaX = b.x - boundaries.minX
      val deltaY = b.y - boundaries.minY
      def inside(x: Int): Int ={
        if(x<0) return 0
        if(x>=sectorPrecision) return sectorPrecision - 1
        x
      }
      val indX =  inside(math.floor( deltaX/(boundaries.width/sectorPrecision) ).toInt)
      val indY =  inside(math.floor( deltaY/(boundaries.height/sectorPrecision) ).toInt)
      matrix( indY*sectorPrecision + indX )+= b
      this
    }
    ...
    def combine(that: SectorMatrix): SectorMatrix = {
      ...
      for (y <- 0 until sectorPrecision) {
        val base = y*sectorPrecision
        for(x <- 0 until sectorPrecision){
          matrix(base+x) = matrix(base+x).combine(that.apply(x,y))
        }
      }
      this
    }
